...
class Omni6DSegmentation(data.Dataset):
    def __init__(self, Omni6dDataset, augs=None, scene_name=None):
        ...
        self.render_template_root = Omni6dDataset['render_template_root']
    ...
    def __getitem__(self, index):
        ...
        for inst in instance_info:
            ...
            if inst['inst_id'] not in unique_inst_ids:
                continue
            obj_label = inst['obj_label']
            obj_id = inst['obj_id']
            mask_id = inst['inst_id']
            image_path = os.path.join(self.render_template_root, obj_label, obj_id, f"image_{render_idx}.png")
        ...
        for obj_label_id in labels_id_non_im:
            obj_label_info = [item for item in self.model_meta["class_list"] if item["label"]==obj_label_id][0]
            obj_label_name = obj_label_info["name"]
            candidate_obj_ids = obj_label_info["instance_ids"]
            # random sample one object id
            obj_id = random.choice(candidate_obj_ids)
            image_path = os.path.join(self.render_template_root, obj_label_name, obj_id, f"image_{render_idx}.png")
            cond_img = cv2.imread(image_path)[:,:,::-1]  # BGR to RGB
            cond_mask = get_mask(cond_img)
            # cond_img augmentation
            cond_img, cond_mask = self._cond_img_augmentation(cond_img, cond_mask)
