...
def get_paths(
    *,
    ...
    paths = {
        ...
        "occlusion": occlusion_tpath.format(**params),
    }
...
def build_gso_index(data_root: str):
    ...
    occlusion_tpath = "{data_root}/{data_patch}/{scene_id}/{scene_name}/{render_name}/occlusion/occlusion_000000.npy"
            ...
            for scene_name in scene_names:
                ...
                for render_name in render_names:
                    paths = get_paths(
                        ...
                        occlusion_tpath=occlusion_tpath,
                    )
                    if paths is None:
                        continue
                    total_images += 1
                    ...
                    # load occlusion info
                    occlusion_info = np.load(paths["occlusion"])
                    ...
                    for idx, key in enumerate(instance_map):
                        ...
                        if value_type != "objects":
                            continue
                        # skip if occlusion ratio is Nan
                        occlusion_ratio = float(occlusion_info[idx][-1])
                        if np.isnan(occlusion_ratio):
                            continue
                        obj_name = value.split("/")[-3]
...
def save_topn_per_object(
    *,
    ...
    for obj_id, entries in obj_iter:
        ...
        # 1) Score each instance by occlusion_ratio.
        scored: list[float, int, int] = []  # (occlusion_ratio, mask_area, data_idx)
        for entry in entries:
            data_idx = int(entry["data_idx"])
            data = total_inst[data_idx]
            inst_id = int(data["inst_id"])
            occlusion_ratio = float(data["occlusion_ratio"])
            mask = cv2.imread(data["mask_path"], cv2.IMREAD_UNCHANGED)
            inst_mask = (mask == inst_id)
            mask_area = int(inst_mask.sum())
            if float(occlusion_ratio) > float(max_ratio) or mask_area < int(min_area):
                continue

            scored.append((occlusion_ratio, mask_area, data_idx))

        if not scored:
            continue

        scored.sort(key=lambda x: x[1]) # based on the mask_area, ascending
        selected_data_idx = [data_idx for _score, _area, data_idx in scored[: int(top_n)]]
...
if __name__ == "__main__":
    ...
    parser.add_argument("--out_dir", default="prepared_cond_gso")
    parser.add_argument("--top_n", type=int, default=100)
    parser.add_argument("--max_ratio", type=float, default=0.05)
    ...
    gso_cfg = cfg.get("GSODataset")
    if gso_cfg is None:
        raise KeyError(f"Missing 'GSODataset' in {args.config}")

    data_root = gso_cfg["data_root"]
    condition_size = int(args.condition_size) 

    start = time.time()
    index = build_gso_index(data_root=data_root)
    logging.info("GSO index built in %.1f sec", time.time() - start)
    logging.info("GSO instance entries: %d", len(index["total_inst"]))
    logging.info("GSO total images: %d", int(index["total_images"]))
