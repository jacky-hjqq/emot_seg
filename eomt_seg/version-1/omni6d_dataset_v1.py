...
class Omni6DSegmentation(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        # double check the mask - mask out the objects not in cond_mask_ids
        valid_mask = np.isin(mask, cond_mask_ids.numpy())
        ...
        # check if mask and cond_mask_ids match
        assert set(torch.unique(mask).tolist()) == set(cond_mask_ids.tolist() + [-1]), f"Mask IDs and cond_mask_ids do not match! mask ids: {torch.unique(mask)}, cond_mask_ids: {cond_mask_ids.tolist()}"
