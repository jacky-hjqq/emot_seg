...
def build_lmo_index(*, data_root: str):
    """Build an instance index for BOP LMO-style datasets.
...
if __name__ == "__main__":
    ...
    parser.add_argument("--out_dir", default="prepared_cond_lmo")
    ...
    lmo_cfg = cfg.get("LMO_Train_Dataset")
    if lmo_cfg is None:
        raise KeyError(f"Missing 'LMO_Train_Dataset' in {args.config}")

    data_root = lmo_cfg["data_root"]
    condition_size = int(args.condition_size)

    index = build_lmo_index(data_root=data_root)

    logging.info("LMO instance entries: %d", len(index["total_inst"]))
